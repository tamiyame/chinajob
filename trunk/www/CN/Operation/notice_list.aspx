<%@ Page Language="C#" MasterPageFile="~/CN/WebMaster/OperationMaster.master" AutoEventWireup="true" CodeFile="notice_list.aspx.cs" Inherits="CN_Operation_notice_list" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="Content" Runat="Server">
                        ...
                        <div class="contents">
                            <table class="board-list">
                                <tr>
                                    <th><%= GetText("CN_Operation_notice_list.aspx_13")%></td>
                                    <th><%= GetText("CN_Operation_notice_list.aspx_14")%></td>
                                    <th><%= GetText("CN_Operation_notice_list.aspx_15")%></td>
                                    <th><%= GetText("CN_Operation_notice_list.aspx_16")%></td>
                                </tr>
<% foreach (ArticleEntity item in ArticleList.Record) { %>
                                <tr>
                                    ...
                                    <td class="col3"><%=item.UserCNName%></td>
...
%>
<%  if (PageNo > 10) { %>
                                            <a href="<%=PageMove("notice_list.aspx", StartPageNo-10)%>" class="icon icon-board-icon-prev"></a>
<% } %>
<%  for (int pageNo = StartPageNo; pageNo <= EndPageNo; pageNo++) { %>
                                            <a href="<%=PageMove("notice_list.aspx", pageNo)%>" <%=pageNo==PageNo ? "class='select'" : "" %>><%=pageNo %></a>
<% } %>
<% if ( EndPageNo < TotalPageNo ) { %>
                                            <a href="<%=PageMove("notice_list.aspx", StartPageNo+10)%>" class="icon icon-board-icon-next"></a>
<% } %>
                                        </div>
